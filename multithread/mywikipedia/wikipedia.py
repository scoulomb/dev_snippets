...
USER_AGENT = 'mywikipedia (https://github.com/goldsmith/Wikipedia/)'
...
def set_lang(prefix):
    ...
    '''
    global API_URL
    API_URL = 'http://' + prefix.lower() + '.mywikipedia.org/w/api.php'
...
def set_rate_limiting(rate_limit, min_wait=timedelta(milliseconds=50)):
    ...
    load on Wikipedia, the number of requests you and other `mywikipedia` users
...
def geosearch(latitude, longitude, title=None, results=10, radius=1000):
    '''
    Do a mywikipedia geo search for `latitude` and `longitude`
...
def languages():
    ...
    Can be inputted to `set_lang` to change the Mediawiki that `mywikipedia` requests
    results from.

    Returns: dict of <prefix>: <local_lang_name> pairs. To get just a list of prefixes,
    use `mywikipedia.languages().keys()`.
    '''
